    ...
    if (input->pixel_format == target_format) {
        ...
        for (int i = 0; i < MAX_PLANES; i++) {
            ...
            if (input->planes[i].buffer) {
                this->buffers[i] = realloc(this->buffers[i], input->height * input->planes[i].stride);
                memcpy(this->buffers[i], input->planes[i].buffer, input->height * input->planes[i].stride);
